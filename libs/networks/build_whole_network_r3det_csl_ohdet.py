...
from libs.detection_oprations.refine_proposal_opr_csl_ohdet import postprocess_detctions
from libs.detection_oprations.anchor_target_layer_without_boxweight import anchor_target_layer
from libs.detection_oprations.refinebox_target_layer_without_boxweight_csl_ohdet import refinebox_target_layer
...
class DetectionNetwork(object):
    ...
    def refine_net(self, feature_pyramid, name):
        ...
        with tf.variable_scope(name):
                ...
                for level in cfgs.LEVEL:
                    ...
                    if cfgs.SHARE_NET:
                        ...
                        scope_list = ['conv2d_3x3_cls', 'conv2d_3x3_reg', 'refine_classification',
                                      'refine_regression', 'rpn_head_cls', 'rpn_angle_cls']
                    else:
                        reuse_flag = None
                        scope_list = ['conv2d_3x3_cls_' + level, 'conv2d_3x3_reg_' + level,
                                      'refine_classification_' + level, 'refine_regression_' + level,
                                      'refine_head_cls_' + level, 'refine_angle_cls_' + level]

                    refine_box_scores, refine_box_probs = self.refine_cls_net(feature_pyramid[level],
                                                                              scope_list, reuse_flag,
                                                                              level)
                    refine_delta_boxes, refine_head_cls, refine_angle_cls = self.refine_reg_net(feature_pyramid[level],
                                                                                                scope_list,
                                                                                                reuse_flag, level)
                    ...
                    refine_head_cls_list.append(refine_head_cls)
            ...
            return refine_delta_boxes_list, refine_scores_list, refine_probs_list, refine_head_cls_list, refine_angle_cls_list
    ...
    def build_whole_detection_network(self, input_img_batch, gtboxes_batch_h, gtboxes_batch_r, gthead_quadrant,
        ...
        if self.is_training:
            with tf.variable_scope('build_loss'):
                ...
                if cfgs.USE_IOU_FACTOR:
                    ...
                    # reg_loss = losses.iou_smooth_l1_loss_1(rpn_box_pred, anchor_states, target_boxes, anchors)
                else:
        ...
        all_box_pred_list, all_cls_prob_list, all_proposal_list, all_head_cls_list, all_angle_cls_list = [], [], [], [], []

        for i in range(cfgs.NUM_REFINE_STAGE):
            box_pred_list, cls_prob_list, proposal_list, head_cls_list, angle_cls_list = self.refine_stage(
                input_img_batch,
                gtboxes_batch_r,
                gthead_quadrant,
                gt_smooth_label,
            ...
            if not self.is_training:
                ...
                all_head_cls_list.extend(head_cls_list)
                all_angle_cls_list.extend(angle_cls_list)
            else:
                all_box_pred_list, all_cls_prob_list, all_proposal_list, all_head_cls_list, all_angle_cls_list = box_pred_list, cls_prob_list, proposal_list, head_cls_list, angle_cls_list
        ...
        with tf.variable_scope('postprocess_detctions'):
            ...
            head_cls = tf.concat(all_head_cls_list, axis=0)
            angle_cls = tf.concat(all_angle_cls_list, axis=0)

            boxes, scores, category, boxes_head, boxes_angle = postprocess_detctions(refine_bbox_pred=box_pred,
                                                                                     refine_cls_prob=cls_prob,
                                                                                     refine_head_prob=tf.sigmoid(
                                                                                         head_cls),
                                                                                     refine_angle_prob=tf.sigmoid(
                                                                                         angle_cls),
                                                                                     anchors=proposal,
                                                                                     is_training=self.is_training)
    ...
    def refine_stage(self, input_img_batch, gtboxes_batch_r, gthead_quadrant, gt_smooth_label, box_pred_list,
                     cls_prob_list, proposal_list, angle_cls_list, feature_pyramid, gpu_id, pos_threshold,
                     neg_threshold, stage, proposal_filter=False):
        ...
        if self.is_training:
            with tf.variable_scope('build_refine_loss{}'.format(stage)):
                refine_labels, refine_target_delta, refine_box_states, refine_target_boxes, refine_target_head_quadrant, refine_target_smooth_label = tf.py_func(
                    ...
                    inp=[gtboxes_batch_r, gthead_quadrant, gt_smooth_label, refine_boxes, pos_threshold, neg_threshold,
                         gpu_id],
                ...
                if cfgs.DATASET_NAME.startswith('DOTA'):
                    head_cls_loss = losses.head_specific_cls_focal_loss(refine_target_head_quadrant, refine_head_cls,
                                                                        refine_box_states, refine_labels,
                                                                        specific_cls=[6, 7, 8, 9, 10, 11])
                else:
                    head_cls_loss = losses.head_focal_loss(refine_target_head_quadrant, refine_head_cls, refine_box_states)
                ...
                self.losses_dict['head_cls_loss{}'.format(stage)] = head_cls_loss * cfgs.HEAD_CLS_WEIGHT
